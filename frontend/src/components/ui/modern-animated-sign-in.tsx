...
const TechOrbitDisplay = memo(function TechOrbitDisplay({
  ...
  return (
    <section className="relative flex h-full w-full flex-col items-center justify-center overflow-hidden rounded-lg">
      <span className="pointer-events-none whitespace-pre-wrap text-center text-7xl font-semibold leading-none text-brand-950">
        {text}
      </span>
...
const FIELDS_PER_ROW: Record<number, string> = {
  1: 'md:grid-cols-1',
  2: 'md:grid-cols-2',
  3: 'md:grid-cols-3',
}
  ...
  const validateForm = (event: FormEvent<HTMLFormElement>) => {
    ...
    fields.forEach(field => {
      ...
      if (field.required && !value) {
        currentErrors[field.label] = field.requiredError ?? `${field.label} is required`
      }
  ...
  return (
    <section className="max-md:w-full flex flex-col gap-4 w-96 mx-auto">
      ...
      <form onSubmit={handleSubmit}>
        <section className={`grid grid-cols-1 ${FIELDS_PER_ROW[fieldPerRow] ?? 'md:grid-cols-1'} mb-4`}>
          {fields.map(field => (
        ...
        >
          <button
            className="bg-gradient-to-br relative group/btn from-brand-600 to-brand-700 block w-full text-white
              rounded-md h-10 font-medium shadow-[0px_1px_0px_0px_#ffffff40_inset,0px_-1px_0px_0px_#ffffff40_inset]
              outline-hidden hover:cursor-pointer disabled:cursor-not-allowed disabled:opacity-60"
            type="submit"
            disabled={isLoading}
          >
            {submitButton} &rarr;
        ...
        {textVariantButton && goTo && (
          <BoxReveal boxColor="var(--skeleton)" duration={0.3}>
            <section className="mt-4 text-center hover:cursor-pointer">
              <button
                type="button"
                className="text-sm text-brand-600 hover:cursor-pointer outline-hidden"
                onClick={goTo}
              >
                {textVariantButton}
              </button>
...
const BottomGradient = () => {
  return (
    <>
      <span className="group-hover/btn:opacity-100 block transition duration-500 opacity-0 absolute h-px w-full -bottom-px inset-x-0 bg-gradient-to-r from-transparent via-fire-500 to-transparent" />
      <span className="group-hover/btn:opacity-100 blur-sm block transition duration-500 opacity-0 absolute h-px w-1/2 mx-auto -bottom-px inset-x-10 bg-gradient-to-r from-transparent via-brand-400 to-transparent" />
    </>
  )
}
